...
export function CustomSelect({ array }: CustomSelectProps) {
    ...
    const options = array;
    console.log('OPCIONES DEL COMPONENTE ---->', options)
    return (
        <div className="relative inline-block text-left w-48 ">
            {/* SELECT BUTTON */}
            <button
                onClick={() => setIsOpen(!isOpen)}
                className="flex tracking-wider justify-between items-center w-full px-2  text-sm font-bold text-gray-900 uppercase"
            >
                {selected}
                ...
                <ul className="absolute w-full bg-white border  border-gray-300 shadow-md rounded-md mt-1 z-10">
                    {options.map((option) => (
                        <li
                            ...
                            className="px-3 py-2 cursor-pointer tracking-wider hover:bg-blue-100 text-xs text-gray-900"
                        >
                            {option}
                        </li>
                    ))}
