...
import { ModalNoParams } from "./components/mod/ModalNoParams"
...
export function GameModule({ domain }: GameModuleProps) {
    ...
    //modal cuando no tengamos codigo por parametros
    ...
    useEffect(() => {
        console.log('CODIGO PARA EL RASPA', code)
        if (!code) {
            setModalParams(true)
            setFetchPrize(false)
        }
    ...
    return (
        ...
        >
            ...
            {modalParams && (
                <ModalNoParams subTitle="No hemos recibido un código de campaña válido. Por favor, verifica con la persona que te compartió el enlace y asegúrate de que sea el correcto." domain={domain} onClick={() => navigate('/client/game/howToGet')} title="Código de campaña no proporcionado" />
            )}
            {modal && (
