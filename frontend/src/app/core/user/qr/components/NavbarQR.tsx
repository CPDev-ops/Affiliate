...
import { IconBackHome } from "./IconQR.tsx";
import { LevelDto } from "../../../../../types/user.ts";
...
export function NavbarQR({ level, domain }: NavbarProp) {
    ...
    return (
        <div className={`flex justify-between items-center  lg:py-2 text-white    gap-2 `}>
            <img src={level !== 0 ? `/images/user/navbar-${domain.toLowerCase()}.png` : `/images/user/navbar-${domain.toLowerCase()}.png`}
                alt="Logo"
                className={`hidden rounded-full ${domain.toUpperCase() !== 'SALTA' ? '  w-72' : '  w-20 sm:w-24 lg:32 '} p-4 py-2 h-full`} />
            {/* HEADER ARRIBA DE TODO */}
            {!isMounted && (
                <div className="p-4 rounded-full bg-zinc-600/70 shadow-md mr-auto ">
                    <IconBackHome level={level} />
                </div>
            )}
            <HiBars3 onClick={openSidebar} size={40} color={`${level !== 0 ? 'white' : 'white'}`} className="mr-4 cursor-pointer  ml-auto" />
            {isMounted && (
                    ...
                    <motion.div variants={sidebarVariants} initial='hidden' animate='visible' exit='exit' transition={{ type: 'tween', duration: 0.3 }} onAnimationComplete={() => {
                        ...
                        <ul className="p-4 uppercase space-y-4 text-gray-800">
                            {navLinks.map((link, index) => (
                                <li key={index} onClick={() => navigate(link.path)} className={`${level !== 0 ? 'text-white hover:text-stone-300' : 'text-white hover:text-stone-300'}  py-2 cursor-pointer `}>{link.title}</li>
                            ))}
                        </ul>
                    </motion.div>
                </ModalLogic>
